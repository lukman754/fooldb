...
function scheduleVisualLayout(fn: () => void, delay = 120) {
  ...
  visualLayoutTimer = setTimeout(() => {
    visualLayoutTimer = null;
    fn();
  }, delay);
}

function layoutMatchesSchema(
  layout: LayoutData | null,
  schema: DatabaseSchema,
) {
  if (!layout || layout.nodes.length !== schema.tables.length) return false;

  const layoutTables = new Set(layout.nodes.map((node) => node.id));
  return schema.tables.every((table) => layoutTables.has(table.name));
}
...
interface DbState {
  ...
  setUmlActors: (
    actors: UmlActor[] | ((prev: UmlActor[]) => UmlActor[]),
  ) => void;
  setUmlUsecases: (
    usecases: UmlUsecase[] | ((prev: UmlUsecase[]) => UmlUsecase[]),
  ) => void;
  setUmlLinks: (links: UmlLink[] | ((prev: UmlLink[]) => UmlLink[])) => void;
  setUmlRelations: (
    relations: UmlRelation[] | ((prev: UmlRelation[]) => UmlRelation[]),
  ) => void;
  mode: AppMode;
  sqlCode: string;
  usecaseCode: string;
...
function syncDslToInteractive(code: string) {
  ...
  parsed.connections.forEach((conn, index) => {
    const isLink =
      actors.some((a) => a.id === conn.from) &&
      usecases.some((u) => u.id === conn.to);
    if (isLink) {
      links.push({
...
export const useDbStore = create<DbState>((set, get) => {
  ...
  return {
    ...
    setUmlActors: (actors) =>
      set((state) => ({
        umlActors:
          typeof actors === "function" ? actors(state.umlActors) : actors,
      })),
    setUmlUsecases: (usecases) =>
      set((state) => ({
        umlUsecases:
          typeof usecases === "function"
            ? usecases(state.umlUsecases)
            : usecases,
      })),
    setUmlLinks: (links) =>
      set((state) => ({
        umlLinks: typeof links === "function" ? links(state.umlLinks) : links,
      })),
    setUmlRelations: (relations) =>
      set((state) => ({
        umlRelations:
          typeof relations === "function"
            ? relations(state.umlRelations)
            : relations,
      })),
    mode: "erd",
    sqlCode: DEFAULT_SQL,
    ...
    frameName: "Diagram",
    setFrameName: (name) => set({ frameName: name }),
    ...
    addVisualTable: (name) => {
      if (!name.trim()) return;
      const existing = get().visualSchema.tables.find(
      ...
      set((state) => ({
        visualSchema: {
          ...state.visualSchema,
          tables: [...state.visualSchema.tables, newTable],
        },
      }));
      ...
      set((state) => ({
        visualSchema: {
          ...
          relationships: state.visualSchema.relationships.map((r) => ({
            ...r,
            sourceTable:
              r.sourceTable === oldName ? newName.trim() : r.sourceTable,
            targetTable:
              r.targetTable === oldName ? newName.trim() : r.targetTable,
          })),
        },
      }));
      scheduleVisualLayout(() => get().triggerVisualLayout());
        ...
        ...(() => {
          ...
          const relationships = state.visualSchema.relationships.map((rel) =>
            newName && newName !== colName && rel.sourceTable === tableName
              ? {
                  ...rel,
                  sourceColumns: rel.sourceColumns.map((c) =>
                    c === colName ? newName : c,
                  ),
                }
              : rel,
          ...
          const layout =
            state.layout && updatedTable
              ? {
                  ...state.layout,
                  nodes: state.layout.nodes.map((node) =>
                    node.table.name === tableName
                      ? { ...node, table: updatedTable! }
                      : node,
                  ),
                  edges: state.layout.edges.map((edge) => ({
                    ...
                    relationship:
                      relationships.find(
                        (rel) => rel.id === edge.relationship.id,
                      ) ?? edge.relationship,
                  })),
                }
              : state.layout;
      ...
      const type: "1:1" | "1:N" | "M:N" =
        sourceCardinality === "one" && targetCardinality === "one"
          ...
          : sourceCardinality === "many" && targetCardinality === "many"
            ...
            : "1:N";
      set((state) => {
        const updatedVisualRelationships = state.visualSchema.relationships.map(
          (rel) =>
            rel.id === relId
              ? { ...rel, type, sourceCardinality, targetCardinality }
              : rel,
        );
        ...
        return {
          visualSchema: {
            ...state.visualSchema,
            relationships: updatedVisualRelationships,
          },
      ...
      set((state) => {
        ...
        const edges = state.layout.edges.map((edge) => {
          if (
            ...
            edge.targetTable !== tableName
          ) {
            return edge;
          }

          connectedEdgeIds.add(edge.id);
          connectedRelIds.add(edge.relationship.id);

          const sourceNode = nodes.find(
            (node) => node.table.name === edge.sourceTable,
          );
          const targetNode = nodes.find(
            (node) => node.table.name === edge.targetTable,
          );
          ...
          return { ...edge, points: [sourceCenter, targetCenter] };
        });

        const relPositions = { ...state.relPositions };
        const customWaypoints = { ...state.customWaypoints };
        connectedRelIds.forEach((relId) => delete relPositions[relId]);
        connectedEdgeIds.forEach((edgeId) => delete customWaypoints[edgeId]);

        if (typeof window !== "undefined") {
          localStorage.setItem(
            "fooldb_rel_positions",
            JSON.stringify(relPositions),
          );
          localStorage.setItem(
            "fooldb_custom_waypoints",
            JSON.stringify(customWaypoints),
          );
        }
    ...
    addVisualFK: (fromTable, toTable) => {
      const state = get();
      const targetTableDef = state.visualSchema.tables.find(
        (t) => t.name === toTable,
      );
      ...
      const fromTableDef = state.visualSchema.tables.find(
        (t) => t.name === fromTable,
      );
      if (!fromTableDef) return;
      const alreadyExists = fromTableDef.columns.some(
        (c) => c.name === fkColName,
      );
      if (!alreadyExists) {
        const fkCol: Column = {
                ...
                ? {
                    ...
                    foreignKeys: [
                      ...t.foreignKeys,
                      {
                        ...
                        referencedColumns: [referencedCol],
                      },
                    ],
                  }
                : t,
            ...
            t.name === rel.sourceTable
              ? {
                  ...
                  columns: t.columns.filter(
                    (c) => !rel.sourceColumns.includes(c.name),
                  ),
                  foreignKeys: t.foreignKeys.filter(
                    (fk) =>
                      !(
                        fk.referencedTable === rel.targetTable &&
                        fk.columns.some((c) => rel.sourceColumns.includes(c))
                      ),
                  ),
                }
              : t,
          ),
          relationships: s.visualSchema.relationships.filter(
            (r) => r.id !== relId,
          ),
        },
      }));
    ...
    setMode: (mode) => {
      const state = get();
      const hasVisualSchema = state.visualSchema.tables.length > 0;
      ...
      if (isDatabaseMode && hasVisualSchema) {
        set({
          mode,
          sqlCode: visualSchemaToSql(state.visualSchema),
          visualSchemaActive: true,
        });
        if (!layoutMatchesSchema(state.layout, state.visualSchema)) {
          get().triggerVisualLayout();
        }
      } else if (mode === "visual" || mode === "uml") {
        set({ mode });
        if (mode === "visual") {
          const cleared = state.hasClearedVisualSchema;
          if (!hasVisualSchema && !cleared && state.schema.tables.length > 0) {
    ...
    triggerParse: async (modeArg, codeArg) => {
      ...
      if (
        (modeArg ?? get().mode) === "visual" ||
        (modeArg ?? get().mode) === "uml"
      ) {
      ...
      try {
        if (
          ...
          const filteredTables = rawSchema.tables.filter(
            (t) => !excluded.includes(t.name.toLowerCase()),
          );
        ...
        set({
          error:
            msg ||
            "An error occurred while parsing and laying out the diagram.",
        });
      }
    },
    ...
    setZoom: (zoom) => {
      if (typeof zoom === "function") {
        set((state) => ({
          zoom: Math.max(0.1, Math.min(3, zoom(state.zoom))),
        }));
      } else {
    ...
    triggerAiLabeling: async () => {
      ...
      if (!apiKey) {
        throw new Error(
          "API Key is missing. Please click the Key icon in the header to set your Gemini API Key.",
        );
      }
      ...
      try {
        ...
        if (!currentSchema || currentSchema.relationships.length === 0) {
          throw new Error(
            "No relationships found in the active schema to label.",
          );
        }
    ...
    initializeStore: (currentProjectId?: string | null) => {
      if (typeof window !== "undefined") {
        ...
        try {
          const savedWaypoints = localStorage.getItem(
            "fooldb_custom_waypoints",
          );
          if (savedWaypoints) {
        ...
        try {
          ...
          if (cachedBuilderState) {
            const cached = JSON.parse(cachedBuilderState) as Partial<
              Pick<
                DbState,
                | "visualSchema"
                | "visualSchemaActive"
            ...
            if (
              cached.visualSchemaActive &&
              cached.visualSchema?.tables?.length
            ) {
              get().triggerVisualLayout();
            } else {
              const code =
    ...
    updateWaypoints: (edgeId, side, waypoints) => {
      set((state) => {
        const existing = state.customWaypoints[edgeId] || {
          sourceWaypoints: [],
          targetWaypoints: [],
        };
        const updated = {
    ...
    updateClassMethod: (tableName, index, methodSignature) => {
      set((state) => {
        ...
        const updated = {
          ...state.classMethods,
          [tableName]: current.map((m, idx) =>
            idx === index ? methodSignature : m,
          ),
        };
        return { classMethods: updated };
      });
