...
import {
  Plus, Trash2, Check, X, Link, Unlink, Code, Copy, ChevronDown, ChevronUp, Database,
} from 'lucide-react';
    ...
    <div className="group flex items-center gap-1.5 px-2 py-1 rounded hover:bg-zinc-800/60 transition-colors">
      ...
      <div className="flex items-center gap-1">
        {(['isPrimaryKey', 'isNullable', 'isUnique'] as const).map((flag) => {
          ...
          const activeColor = flag === 'isPrimaryKey' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-600/40' 
            : flag === 'isUnique' ? 'bg-purple-500/20 text-purple-400 border-purple-600/40'
          ...
          return (
            <button
              ...
              disabled={isLocked}
              onClick={() => {
                if (flag === 'isNullable') onUpdate({ isNullable: !col.isNullable });
              ...
              title={flag === 'isPrimaryKey' ? 'Primary Key' : flag === 'isNullable' ? 'Not Null' : 'Unique'}
            >
              {labels[flag]}
            </button>
          );
...
// Table Card
// ────────────────────────────────────────────────
function TableCard({ tableName }: { tableName: string }) {
  const {
    visualSchema,
    renameVisualTable, removeVisualTable,
    addVisualColumn, removeVisualColumn, updateVisualColumn,
    addVisualFK, removeVisualRelation,
  } = useDbStore();
  ...
  const tableRelations = visualSchema.relationships.filter((r) => r.sourceTable === tableName);
  const fkTargets = visualSchema.tables
    .filter((t) => t.name !== tableName && !tableRelations.some((r) => r.targetTable === t.name))
    .map((t) => t.name);
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/80 backdrop-blur shadow-md overflow-hidden">
      {/* Header */}
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          {fkTargets.length > 0 && (
            <div className="relative">
              <button
                onClick={() => setShowFkMenu(!showFkMenu)}
              ...
              {showFkMenu && (
                <>
                  <div className="fixed inset-0 z-20" onClick={() => setShowFkMenu(false)} />
                  <div className="absolute left-0 mt-1 w-44 rounded-lg border border-zinc-700 bg-zinc-900 p-1 shadow-lg z-30">
                    <div className="px-2 py-1 text-[10px] text-zinc-500 font-medium">References table →</div>
                    {fkTargets.map((target) => (
                      <button
                        key={target}
                        onClick={() => { addVisualFK(tableName, target); setShowFkMenu(false); }}
        ...
        <div>
          ...
          <div className="py-1">
            {table.columns.map((col) => {
              const isFkCol = table.foreignKeys.some((fk) => fk.columns.includes(col.name));
              return (
                <ColumnRow
                  key={col.name}
                  tableName={tableName}
          ...
          <div className="px-2 pb-2">
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 rounded-md border border-dashed border-zinc-700 py-1 text-[10px] text-zinc-500 hover:border-blue-600 hover:text-blue-400 transition-colors"
            >
          ...
          {tableRelations.length > 0 && (
            <div className="border-t border-zinc-800 px-2 py-1.5">
              <div className="text-[9px] font-semibold text-zinc-600 uppercase tracking-widest mb-1">Relations</div>
              {tableRelations.map((rel) => (
                ...
                >
                  ...
                  <span className="font-semibold text-zinc-300">{rel.targetTable}</span>
                  <span className="text-[9px] text-zinc-600 px-1 rounded bg-zinc-800">1:N</span>
                  <button
...
// SQL Export Modal
// ────────────────────────────────────────────────
function SqlExportModal({ sql, onClose }: { sql: string; onClose: () => void }) {
      ...
      <div className="relative w-[640px] max-h-[80vh] rounded-xl border border-zinc-700 bg-zinc-900 shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-zinc-100">Export SQL DDL</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
        ...
        <div className="flex-1 overflow-auto p-4">
          <pre className="text-xs text-zinc-300 font-mono leading-relaxed whitespace-pre-wrap">{sql}</pre>
        </div>
      </div>
    </div>
  );
    ...
    <div className="h-full flex flex-col bg-zinc-950 overflow-hidden">
      ...
      <div className="flex items-center gap-2 border-b border-zinc-800 px-4 py-2.5 bg-zinc-950 shrink-0">
        <div className="flex-1">
          <span className="text-xs font-semibold text-zinc-300">Visual ERD Builder</span>
          <span className="ml-2 text-[10px] text-zinc-600">
            {visualSchema.tables.length} table{visualSchema.tables.length !== 1 ? 's' : ''}
            {' · '}
            {visualSchema.relationships.length} relation{visualSchema.relationships.length !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 border-b border-zinc-800 px-4 py-2 bg-blue-950/20 shrink-0">
          ...
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === 'Escape') { setShowAddInput(false); setNewTableName(''); }
            }}
          ...
          />
          <button onClick={handleAddTable} className="text-blue-400 hover:text-blue-300 transition-colors">
            <Check className="h-4 w-4" />
          </button>
          <button onClick={() => { setShowAddInput(false); setNewTableName(''); }} className="text-zinc-600 hover:text-zinc-400 transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {visualSchema.tables.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-6">
            ...
            <div>
              <p className="text-sm font-semibold text-zinc-400">No tables yet</p>
              <p className="text-xs text-zinc-600 mt-1 leading-relaxed">
                Click <strong className="text-zinc-400">+ Add Table</strong> to start building your ERD visually — no SQL needed!
              </p>
            </div>
            <button
              onClick={() => setShowAddInput(true)}
          ...
          visualSchema.tables.map((table) => (
            <TableCard key={table.name} tableName={table.name} />
          ))
        )}
      </div>
      ...
      {/* SQL Export Modal */}
      {showSqlModal && (
        <SqlExportModal sql={sqlOutput} onClose={() => setShowSqlModal(false)} />
      )}
    </div>
  );
}
