...
export async function POST(request: Request) {
  try {
    const { apiKey, schema }: { apiKey: string; schema: DatabaseSchema } = await request.json();
    if (!apiKey) {
      return NextResponse.json({ error: 'API Key is empty.' }, { status: 400 });
    }
    if (!schema) {
      return NextResponse.json({ error: 'Database Schema is missing.' }, { status: 400 });
    }

    const tablesSummary = schema.tables
      .map((t: Table) => {
        const cols = t.columns.map((c: Column) => `${c.name} (${c.type})`).join(', ');
